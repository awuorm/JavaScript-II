...
runners.forEach((element) => fullNames.push(` Full name is ${element.first_name}  ${element.last_name}`));
...
let runnersLargeSizeShirt = [];
 ...
 const largeShirts = function(tshirt) { 
   if (tshirt.shirt_size === "3XL") {
     return true;
}
    return false;
}
...
let donations = runners.map(function(tickets) {
  return tickets.donation;
})
ticketPriceTotal = donations.reduce(function(acc, value) {
  return acc + value;
})
